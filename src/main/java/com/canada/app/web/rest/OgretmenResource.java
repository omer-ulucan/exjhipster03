...
/**
 * REST controller for managing {@link com.canada.app.domain.Ogretmen}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class OgretmenResource {
    ...
    public ResponseEntity<Ogretmen> partialUpdateOgretmen(
        ...
        Optional<Ogretmen> result = ogretmenRepository
            ...
            .map(existingOgretmen -> {
                if (ogretmen.getAdiSoyadi() != null) {
                    existingOgretmen.setAdiSoyadi(ogretmen.getAdiSoyadi());
                }
                if (ogretmen.getBrans() != null) {
                    existingOgretmen.setBrans(ogretmen.getBrans());
                }
     ...
     */
    @GetMapping("/ogretmen")
    public List<Ogretmen> getAllOgretmen(@RequestParam(required = false) String filter) {
        if ("sinif-is-null".equals(filter)) {
            log.debug("REST request to get all Ogretmens where sinif is null");
            return StreamSupport
                .stream(ogretmenRepository.findAll().spliterator(), false)
                .filter(ogretmen -> ogretmen.getSinif() == null)
