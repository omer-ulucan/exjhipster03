...
/**
 * REST controller for managing {@link com.canada.app.domain.Sinif}.
 */
@RestController
@RequestMapping("/api")
@Transactional
public class SinifResource {
    ...
    public ResponseEntity<Sinif> updateSinif(@PathVariable(value = "id", required = false) final Long id, @RequestBody Sinif sinif)
    ...
    public ResponseEntity<Sinif> partialUpdateSinif(@PathVariable(value = "id", required = false) final Long id, @RequestBody Sinif sinif)
        throws URISyntaxException {
        ...
        Optional<Sinif> result = sinifRepository
            ...
            .map(existingSinif -> {
                if (sinif.getSinifAdi() != null) {
                    existingSinif.setSinifAdi(sinif.getSinifAdi());
                }
                if (sinif.getSinifKodu() != null) {
                    existingSinif.setSinifKodu(sinif.getSinifKodu());
                }
                if (sinif.getBrans() != null) {
                    existingSinif.setBrans(sinif.getBrans());
                }
    ...
    public ResponseEntity<Sinif> getSinif(@PathVariable Long id) {
        ...
        Optional<Sinif> sinif = sinifRepository.findById(id);
